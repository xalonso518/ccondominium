...
@Component({
  selector: 'app-tipos-cumplimiento',
  templateUrl: './tipos-cumplimiento.component.html',
  styleUrls: ['./tipos-cumplimiento.component.css'],
  providers: [TipoCumplimientoService]
})
export class TiposCumplimientoComponent implements OnInit {
  displayedColumns: string[] = ['nombre', 'importe', 'nivel', 'estado', 'accion'];
  tipos: TipoCumplimientoView[] = new Array();
  dataSource = new MatTableDataSource(this.tipos);
  @ViewChild(MatSort, {static: true}) sort: MatSort;

  constructor(
    private identityUserService: IdentityUserService,
    private tiposService: TipoCumplimientoService,
  ...
  buscar(): void {
    ...
    this.tiposService.getTiposCumplimientosTodos().subscribe(
        res => {
  ...
  openAgregar(): void {
    const dialogRef = this.dialog.open(AgregarTipoCumplimientoComponent, {
      disableClose: false, autoFocus: false, width: '550px',  data: {isAgregar : true, tipo: null}
    });
  ...
  openEditar(tipo: TipoCumplimientoView): void {
    const dialogRef = this.dialog.open(AgregarTipoCumplimientoComponent, {
      // tslint:disable-next-line: object-literal-shorthand
      disableClose: false, autoFocus: false, width: '550px',  data: {isAgregar : false, tipo: tipo}
    });
  ...
  cambiarEstado(tipo: TipoCumplimientoView, estado: number) {
    // tslint:disable-next-line: one-variable-per-declaration
    const nEstado = new TipoCumplimientoEstado();
    nEstado._id = tipo._id;
    nEstado.estado = estado.toString();

    this.loadingService.show();
    this.tiposService.cambioEstadoTipoCumplimiento(nEstado).subscribe(
    registro => {
        this.loadingService.hide();
